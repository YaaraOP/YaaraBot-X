...
@yaarabot.on(admin_cmd(pattern="botgban ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="botgban ?(.*)", allow_sudo=True))
async def _(event):
...
@yaarabot.on(admin_cmd(pattern="botungban ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="botungban ?(.*)", allow_sudo=True))
async def _(event):
    if Config.G_BAN_LOGGER_GROUP is None:
        await eor(
            event,
