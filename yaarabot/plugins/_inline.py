...
from yaarabot import ALIVE_NAME, CMD_HELP, CMD_LIST, CUSTOM_PMPERMIT, bot
from yaarabot.plugins import telestats
...
TELEPIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://te.legra.ph/file/da37f14b332a1691722cf.jpg"
)
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
myid = bot.uid
...
MESAG = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "`YaaraBot PM security! Please wait for me to approve you. 😊"
)
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("`Userbot"):
            ...
            result = builder.article(
                title="Stats",
                text=f"**YaaraBot Stats For [{DEFAULTUSER}](tg://user?id={myid})**\n\n__Bot is functioning normally, master!__\n\n(c) @MissKaurSupport",
                buttons=[
                    [custom.Button.inline("Stats", data="statcheck")],
                    [Button.url("Repo", "https://github.com/YaaraOP/YaaraBot")],
                    [
                        Button.url(
                            "Deploy Now!",
                            "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FYaaraOP%2FYaaraBot&template=https%3A%2F%2Fgithub.com%2FYaaraOP%2FYaaraBot",
                        )
                    ],
                ],
            )
            ...
            result = builder.article(
                title="Repository",
                text=f"YaaraBot - Telegram Userbot.",
                buttons=[
                    [
                        Button.url("Repo", "https://github.com/YaaraOP/YaaraBot"),
                        Button.url(
                    ...
                    [Button.url("Support", "https://t.me/MissKaurSupport")],
                ],
            )
        else:
            result = builder.article(
                "Source Code",
                text="**Welcome to YaaraBot**\n\n`Click below buttons for more`",
                buttons=[
                    [custom.Button.url("Creator👨‍🦱", "https://t.me/KashDaYash")],
                    [
                        custom.Button.url(
                            "👨‍💻Source Code‍💻", "https://github.com/YaaraOP/YaaraBot"
                        ),
                        custom.Button.url(
                            "Deploy 🌀",
                            "https://dashboard.heroku.com/new?template=https%3A%2F%2Fgithub.com%2FYaaraOP%2FYaaraBot",
                        ),
                    ],
                    [
                        custom.Button.url(
                            "Updates and Support Group↗️", "https://t.me/MissKaurSupport"
                        )
                    ],
                ],
                link_preview=False,
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = (
                "Please get your own Userbot from @YaaraBotHelp , and don't use mine!"
            )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert = "Please get your own userbot from @MissKaurSupport "
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
