...
from yaarabot import CMD_HELP, bot
...
@yaarabot.on(admin_cmd(pattern="btweet(?: |$)(.*)"))
@yaarabot.on(sudo_cmd(pattern="btweet(?: |$)(.*)", allow_sudo=True))
async def teletweet(yaarabot):
    # """Creates random anime sticker!"""
    what = yaarabot.pattern_match.group(1)
    if not what:
        if yaarabot.is_reply:
            what = (await yaarabot.get_reply_message()).message
        else:
            await eor(yaarabot, "`Tweets must contain some text, pero!`")
            return
    sticcers = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(what))}")
    await sticcers[0].click(
        yaarabot.chat_id,
        reply_to=yaarabot.reply_to_msg_id,
        silent=True if yaarabot.is_reply else False,
        hide_via=True,
    )
    await yaarabot.delete()
...
@yaarabot.on(admin_cmd(pattern="tweet ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="tweet ?(.*)"))
async def handler(event):
    ...
    try:
        ...
        await yaarabot(tweetit)
    except BaseException:
        pass
    mssg = deEmojify(mssg)
    pic_tweet = await tweet(uname, mssg)
    await yaarabot.send_file(event.chat_id, pic_tweet, reply_to=reply_to)
    await event.delete()
