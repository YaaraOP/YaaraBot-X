...
from yaarabot.plugins.sql_helper.welcome_sql import (
    add_welcome_setting,
...
@yaarabot.on(admin_cmd(pattern="savewelcome"))  # pylint:disable=E0602
@yaarabot.on(sudo_cmd(pattern="savewelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@yaarabot.on(admin_cmd(pattern="clearwelcome"))  # pylint:disable=E0602
@yaarabot.on(sudo_cmd(pattern="clearwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
...
@yaarabot.on(admin_cmd(pattern="listwelcome"))  # pylint:disable=E0602
@yaarabot.on(sudo_cmd(pattern="listwelcome", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    cws = get_current_welcome_settings(event.chat_id)
