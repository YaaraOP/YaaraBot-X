...
@yaarabot.on(admin_cmd(pattern="fstat ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
        ...
        async with borg.conversation(bot) as conv:
            try:
                ...
                if "Looks like" in audio.text:
                    ...
                    audio = await conv.get_response()
                    await yaarabot.send_file(
                        event.chat_id,
                        audio,
                        caption=f"List of feds {user} has been banned in.\n\nCollected using YaaraBot.",
                    )
                else:
...
@yaarabot.on(admin_cmd(pattern="fedinfo ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
