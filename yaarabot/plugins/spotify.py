...
async def _(event):
    try:
        await yaarabot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
...
"""
    try:
        async for event in yaarabot.iter_messages(
            chat, search=name, limit=1, filter=InputMessagesFilterMusic
        ):
            ok = cap.format(event.message, PROF)
            await yaarabot.delete_messages(current_chat, current_msg)
            await yaarabot.send_file(current_chat, event, caption=ok)
