...
from yaarabot import CMD_HELP, CMD_HNDLR

Heroku = heroku3.from_key(Var.HEROKU_API_KEY)
heroku_api = "https://api.heroku.com"


@yaarabot.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
@yaarabot.on(admin_cmd(pattern="usage"))
@yaarabot.on(sudo_cmd(pattern="usage", allow_sudo=True))
...
@yaarabot.on(admin_cmd(pattern="info heroku"))
@yaarabot.on(sudo_cmd(pattern="info heroku", allow_sudo=True))
...
@yaarabot.on(admin_cmd(outgoing=True, pattern=r"logs"))
@yaarabot.on(sudo_cmd(allow_sudo=True, pattern=r"logs"))
async def _(givelogs):
    ...
    with open("logs-yaarabot.txt", "w") as log:
    ...
    await givelogs.client.send_file(
        ...
        caption=f"**Heroku** YaaraBot Logs.\nPasted [here]({url}) too!",
    )
    await eor(givelogs, "Heroku Logs Incoming!!")
    await asyncio.sleep(5)
    await givelogs.delete()
    return os.remove("logs-yaarabot.txt")
