...
import re
from yaarabot.plugins.mybot import *
from telethon import events, Button
import heroku3
import asyncio
import os
import requests
from yaarabot.plugins.mybot.sql.blacklist_sql import all_bl_users
from yaarabot.plugins import TELE_NAME
from yaarabot.plugins.mybot.sql.userbase_sql import add_to_userbase, present_in_userbase, full_userbase
from datetime import datetime
from telethon import events
from yaarabot.yaarabotConfig import Var, Config
from telegraph import Telegraph, upload_file
from yaarabot import CUSTOM_PMPERMIT
    ...
    if LOAD_MYBOT == "False":
        ...
        if BOT_PIC:
            await tgbot.send_file(event.chat_id,
                                  ...
                                  buttons=[
                                      [Button.url(
                                          "YaaraBot", url="https://github.com/YaaraOP/YaaraBot")],
                                      [Button.inline(
                                          "Whats this?", data="yaarabot")]
                                  ]
                                  )
        else:
            await tgbot.send_message(event.chat_id,
    ...
    await tgbot.send_message(event.chat_id,
                             ...
                             buttons=[
                                 ...
                                 [Button.inline("Broadcast",
                                                data="telebroad")],
                                 [Button.url("Support",
                                             url="https://t.me/MissKaurSupport")]
                             ])
...
async def logs(event):
    ...
    await tgbot.send_file(
        ...
        "logs.txt",
        reply_to=event.id,
        caption="**Heroku** YaaraBot Logs",
        buttons=[
            [Button.url("View Online", f"{url}")],
            [Button.url("Crashed?", "t.me/YaaraBotHelpChat")]
        ])
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"yaarabot"))
          )  # pylint: disable=oof
async def settings(event):
    await event.delete()
    await tgbot.send_message(event.chat_id,
...
async def settings(event):
    await event.edit("Browse through the available options:",
                     buttons=[
                         [(Button.url("Repository", url="https://github.com/YaaraOP/YaaraBot")),
                          (Button.url("Deploy", url="https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FYaaraOP%2FYaaraBot%2F&template=https%3A%2F%2Fgithub.com%2FYaaraOP%2FYaaraBot"))],
                         [Button.url("Support",
                                     url="https://t.me/MissKaurSupport")]
                     ])
