...
@yaarabot.on(admin_cmd(pattern="zip"))
@yaarabot.on(sudo_cmd(pattern="zip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@yaarabot.on(admin_cmd(pattern="unzip"))
@yaarabot.on(sudo_cmd(pattern="unzip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await eor(event, "Processing ...")
