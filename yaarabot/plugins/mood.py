...
from yaarabot import ALIVE_NAME, CMD_HELP
from yaarabot.utils import admin_cmd

TELENAME = ALIVE_NAME if ALIVE_NAME else "YaaraBot"

# set your mood


@yaarabot.on(admin_cmd(pattern="mood ((.|\n)*)"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
...
@yaarabot.on(admin_cmd(pattern="resetmood"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
