...
@yaarabot.on(admin_cmd(pattern="getqr"))
@yaarabot.on(sudo_cmd(pattern="getqr", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
...
@yaarabot.on(admin_cmd(pattern="makeqr ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="makeqr ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
