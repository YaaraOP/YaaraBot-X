...
@yaarabot.on(admin_cmd(pattern="time ?(.*)"))  # pylint:disable=E0602
@yaarabot.on(sudo_cmd(pattern="time ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    await borg.send_file(  # pylint:disable=E0602
        event.chat_id,
        required_file_name,
        caption="YaaraBot: Powered by @YaaraBotHelp",
        # Courtesy: @ManueI15
...
@yaarabot.on(admin_cmd(pattern="gtime (.*)"))  # pylint:disable=E0602
@yaarabot.on(sudo_cmd(pattern="gtime (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
