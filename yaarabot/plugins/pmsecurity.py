...
import yaarabot.plugins.sql_helper.pmpermit_sql as pmpermit_sql
from yaarabot import ALIVE_NAME, CMD_HELP, CUSTOM_PMPERMIT, bot
...
@yaarabot.on(admin_cmd(pattern="a ?(.*)"))
@yaarabot.on(admin_cmd(pattern="approve ?(.*)"))
...
@yaarabot.on(admin_cmd(pattern="block ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@yaarabot.on(admin_cmd(pattern="da ?(.*)"))
@yaarabot.on(admin_cmd(pattern="disapprove ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@yaarabot.on(admin_cmd(pattern="listapproved"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    approved_users = pmpermit_sql.get_all_approved()
    APPROVED_PMs = "[YaaraBot] Currently Approved PMs\n"
    ...
    if len(APPROVED_PMs) > 4095:
        with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
            ...
            await event.client.send_file(
                ...
                force_document=True,
                allow_cache=False,
                caption="[YaaraBot]Current Approved PMs",
                reply_to=event,
            )
            await event.delete()
    else:
...
if NEEDIT == "on":
    ...
    @yaarabot.on(events.NewMessage(incoming=True))
    async def on_new_private_message(event):
