...
@yaarabot.on(ChatAction)
async def handler(tele):
    if tele.user_joined or tele.user_added:
        try:
            from yaarabot.plugins.sql_helper.gmute_sql import is_gmuted
...
@yaarabot.on(admin_cmd(pattern="gban(?: |$)(.*)"))
async def gspider(rk):
    ...
    if user:
        ...
        try:
            from yaarabot.plugins.sql_helper.gmute_sql import gmute
        except BaseException:
            pass
        try:
...
@yaarabot.on(admin_cmd(pattern="ungban(?: |$)(.*)"))
async def gspider(rk):
    lazy = rk
    sender = await lazy.get_sender()
    me = await lazy.client.get_me()
    ...
    if user:
        ...
        try:
            from yaarabot.plugins.sql_helper.gmute_sql import ungmute
        except BaseException:
            pass
        try:
