...
from yaarabot.utils import admin_cmd, load_module, remove_plugin

from .. import ALIVE_NAME

DELETE_TIMEOUT = 5
thumb_image_path = "./resources/YaaraBot.jpeg"
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "YaaraBot User"


@yaarabot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@yaarabot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    the_plugin_file = "./yaarabot/plugins/{}.py".format(input_str)
...
@yaarabot.on(admin_cmd(pattern="install"))
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "yaarabot/plugins/",  # pylint:disable=E0602
                )
            )
            if "(" not in downloaded_file_name:
                ...
                await event.edit(
                    "YaaraBot Succesfully Installed The Plugin `{}`".format(
...
@yaarabot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
async def unload(event):
    ...
    try:
        ...
        await event.edit(f"YaaraBot has successfully unloaded {shortname}")
    except Exception as e:
        await event.edit(
            "YaaraBot has successfully unloaded {shortname}\n{}".format(
                shortname, str(e)
            )
        )


@yaarabot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
async def load(event):
    ...
    try:
        ...
        await event.edit(f"YaaraBot has successfully loaded {shortname}")
    except Exception as e:
        await event.edit(
            f"YaaraBot could not load {shortname} because of the following error.\n{str(e)}"
        )


@yaarabot.on(admin_cmd(pattern=r"installall$"))
async def install(event):
    if event.fwd_from:
        return
    ...
    for ixo in total_doxx:
        ...
        downloaded_file_name = await event.client.download_media(
            await event.client.get_messages(event.chat_id, ids=mxo), "yaarabot/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
