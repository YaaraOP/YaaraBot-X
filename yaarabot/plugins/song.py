...
# By @YaaraOP. Thanks to @AvinashReddy for the ytdl base.
...
@yaarabot.on(admin_cmd(pattern="song (.*)"))
async def download_video(tele):
    ...
    try:
        ...
        await yaarabot(JoinChannelRequest(sung))
    except BaseException:
        pass
    upteload = """
Uploading...
...
@yaarabot.on(admin_cmd(pattern="vsong (.*)"))
async def download_video(tele):
