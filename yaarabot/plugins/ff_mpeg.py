...
from yaarabot.utils import admin_cmd, progress

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "uniborg.media.ffmpeg"


@yaarabot.on(admin_cmd(pattern="ffmpegsave"))
@yaarabot.on(sudo_cmd(pattern="ffmpegsave", allow_sudo=True))
...
@yaarabot.on(admin_cmd(pattern="ffmpegtrim"))
@yaarabot.on(sudo_cmd(pattern="ffmpegtrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
