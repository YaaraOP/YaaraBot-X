...
# Media Afk, Time, by @YaaraOP
"""
AFK Plugin for YaaraBot
Syntax: .afk REASON
"""
import asyncio
import os
        ...
        if Var.PRIVATE_GROUP_ID:
            ...
            if not event.is_private:
                ...
                try:
                    ...
                    await yaarabot.send_message(
                        Var.PRIVATE_GROUP_ID,
                        f"Please add {MYBOT} here for afk tags to work.",
                    )


@yaarabot.on(admin_cmd(pattern=r"afk ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK
    ...
    if not USER_AFK:
        if event.reply_to_msg_id:
            ...
            media = await yaarabot.download_media(reply_message, "AFK_media")
            try:
...
@yaarabot.on(events.NewMessage(outgoing=True))
