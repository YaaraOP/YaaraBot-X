...
@yaarabot.on(admin_cmd(pattern="ugdrive ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@yaarabot.on(admin_cmd(pattern="drivesch ?(.*)"))
async def sch(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@yaarabot.on(admin_cmd(pattern="gdrivedir ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    if CLIENT_ID is None or CLIENT_SECRET is None:
...
@yaarabot.on(admin_cmd(pattern="gfolder ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
