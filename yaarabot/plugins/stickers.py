...
@yaarabot.on(admin_cmd(outgoing=True, pattern="kang"))
@yaarabot.on(sudo_cmd(pattern="kang", allow_sudo=True))
async def kang(args):
    ...
    if photo:
        ...
        packnick = (
            f"{telename} Vol.{pack}"
            if telename
            else f"@{user.username}'s YaaraBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
...
@yaarabot.on(admin_cmd(outgoing=True, pattern="stkrinfo"))
@yaarabot.on(sudo_cmd(pattern="stkrinfo", allow_sudo=True))
