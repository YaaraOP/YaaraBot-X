...
from yaarabot.plugins.sql_helper.locks_sql import get_locks, is_locked, update_lock
from yaarabot.utils import admin_cmd


@yaarabot.on(admin_cmd(pattern=r"lock( (?P<target>\S+)|$)"))
@yaarabot.on(sudo_cmd(pattern=r"lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@yaarabot.on(admin_cmd(pattern="unlock ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="unlock ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@yaarabot.on(admin_cmd(pattern="currenabledlocks"))
@yaarabot.on(sudo_cmd(pattern="currenabledlocks", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
...
@yaarabot.on(events.MessageEdited())  # pylint:disable=E0602
@yaarabot.on(events.NewMessage())  # pylint:disable=E0602
...
@yaarabot.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"
