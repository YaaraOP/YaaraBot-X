...
@yaarabot.on(admin_cmd(pattern=r"remove\.bg ?(.*)"))
@yaarabot.on(sudo_cmd(pattern=r"remove\.bg ?(.*)", allow_sudo=True))
async def _(event):
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "YaaraBot-Rmbg.png"
        ...
        await eor(
            event,
            "Removed dat annoying Backgroup in {} seconds, powered by @YaaraBotHelp".format(
                ms
            ),
        )
    else:
