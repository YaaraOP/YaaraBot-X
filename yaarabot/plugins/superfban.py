...
# By @HeisenbergTheDanger, @KashDaYash


@yaarabot.on(admin_cmd("superfban ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await yaarabot.download_media(
    ...
    try:
        ...
        if FBAN == "@HeisenbergTheDanger" or FBAN == "@YaaraOP":
            await event.edit("Something went wrong.")
            return
    ...
    if not len(fedList):
        for a in range(3):
            async with yaarabot.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await yaarabot.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.EXCLUDE_FED and fed in excludeFed:
            await yaarabot.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await yaarabot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await yaarabot.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
...
@yaarabot.on(admin_cmd("superunfban ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    for fed in fedList:
        await yaarabot.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(5)
        await yaarabot.send_message(chat, f"/unfban {FBAN}")
        await asyncio.sleep(5)
