# Made For DARK COBRA and YAARABOT...
# Made by team cobra with @YaaraOP
...
@yaarabot.on(admin_cmd(pattern="sg ?(.*)"))
@yaarabot.on(sudo_cmd(pattern="sg ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with event.client.conversation(chat) as conv:
        ...
        if response1.text.startswith("No records found"):
            ...
            await event.delete()
            await yaarabot.send_message(event.chat_id, response2.message)

            await yaarabot.send_message(event.chat_id, response3.message)
